...
| NFR1.2 | 頁面載入時間 | < 2s | 🟡 部分完成 |
| NFR1.3 | NDI 輸出延遲 | < 50ms | 🔴 Not Started |
| NFR1.4 | AI 辨識回應時間 | < 1s | 🟡 部分完成 |

> **NFR1.1 備註**：WebSocket 即時同步已實作，延遲符合 <100ms 目標。
> **NFR1.2 備註**：Display 頁面 114 kB First Load JS 達標；Controller 頁面 675 kB 在慢速網路下可能超標，需 `next/dynamic` 懶載入優化。
> **NFR1.4 備註**：端到端延遲 250-860ms，條件性達標。STT 雲端辨識佔延遲 80% 以上，取決於 Provider 網路環境。
...
| NFR5.2 | 支援 WebSocket Secure (WSS) | ✅ 完成 |
| NFR5.3 | 響應式設計支援 320px - 4K | 🟡 部分完成 |

> **NFR5.2 備註**：Production 環境設定已使用 wss:// 協議。
> **NFR5.3 備註**：320px-1280px 範圍覆蓋良好（三級 RWD 佈局），2K-4K 高解析度端缺乏驗證和針對性適配。
